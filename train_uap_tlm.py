...
def train(param):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(data_set)):
        ...
        train_loader = torch.utils.data.DataLoader(data_set, batch_size=batch_size, sampler=train_subsampler)
        val_loader = torch.utils.data.DataLoader(data_set, batch_size=batch_size, sampler=val_subsampler)
        test_loader = torch.utils.data.DataLoader(data_set, batch_size=batch_size, sampler=test_subsampler)
        ...
        for i in range(num_epoch):
            ...
            if param.attack_type == 'non-targeted':
                if val_accuracy < best_val_accuracy:
                    ...
                    uap_file_name = param.uap_path + '_uap_tlm_non_targeted_fold%d_25.npy' % fold
                ...
                if val_accuracy > best_val_accuracy: # If attack_type is targeted, largest acc means that attack is successful!
                    ...
                    uap_exam = adv_exam_cuda.cpu().detach().numpy()[0, :, :, :]
                    # save perturbation
                    uap_file_name = param.uap_path + '_uap_tlm_%d_targeted_fold%d_25.npy' % (param.attack_target, fold)
                    np.save(uap_file_name, uap_exam)
                    print(uap_file_name, 'Saved best perturbation!')
    ...
    #
    # if param.attack_type == 'targeted':
    #     np.savetxt(param.uap_path + '_tlm_result_target%d_fold.txt' % param.attack_target, result_list, fmt='%1.4f')
    #     print('saved at' + param.uap_path + '_tlm_result_target%d_fold.txt' % param.attack_target)
    # elif param.attack_type == 'non-targeted':
    #     np.savetxt(param.uap_path + '_tlm_result_non_target_fold.txt', result_list, fmt='%1.4f')
    #     print('saved at' + param.uap_path + '_tlm_result_non_target_fold.txt')
