...
def train(param):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(subject_list)):
        ...
        save_file_name = pth_path + param.dataset + '_net_condition%d_subj.pth' % (fold)
                ...
                while flag.sum() != len(model_list) * (param.num_class + 1):
                    ...
                    if flag[flag_index] == 1:
                        ...
                        for cii in range(param.num_channel):
                            train_x_adv[:, :, cii, :] = train_x_adv[:, :, cii, :].cpu().clone().clamp(
        ...
        for model_idx in range(len(model_list)):
            ...
            for attack_type_idx in range(6, 6 + (param.num_class + 1)):
                ...
                for test_x, test_y in test_loader:
                    ...
                    # Do clamping per channel
                    for cii in range(param.num_channel):
                        test_x_adv[:, :, cii, :] = test_x_adv[:, :, cii, :].cpu().clone().clamp(
