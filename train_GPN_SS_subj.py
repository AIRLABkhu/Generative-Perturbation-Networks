...
def train(param):
        ...
        for i in range(num_epoch):
            ...
            if i != 0 and (i + 1) % 5 == 0:
                if param.attack_type == 'non-targeted':
                    ...
                    save_file_name = param.uap_path + 'air_uap_net_nt_fold%d_subj.pth' % fold
                    ...
                    # if val_accuracy > best_val_accuracy:
                    #     best_val_accuracy = val_accuracy
                    save_file_name = param.uap_path + 'air_uap_net_t%d_fold%d_subj.pth' % (param.attack_target, fold)
                    torch.save(generator.state_dict(), save_file_name)
                    print('Saved best model at ' + save_file_name)
                    cnt = 0
    ...
    if param.attack_type == 'targeted':
        np.savetxt(param.uap_path + '_air_net_result_target%d_fold_subj.txt' % param.attack_target, result_list, fmt='%1.4f')
        print('saved at' + param.uap_path + '_air_net_result_non_target_fold%d_fold_subj.txt'% param.attack_target)
    elif param.attack_type == 'non-targeted':
        np.savetxt(param.uap_path + '_air_net_result_non_target_fold_subj.txt', result_list, fmt='%1.4f')
        print('saved at' + param.uap_path + '_air_net_result_non_target_fold_subj.txt')
