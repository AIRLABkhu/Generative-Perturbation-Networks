...
def train(param):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(data_set)):
        ...
        train_loader = torch.utils.data.DataLoader(data_set, batch_size=param.batch_size, sampler=train_subsampler,
                                                   num_workers=12)
        ...
        save_file_name = pth_path + param.dataset + '_multi%d.pth' % (fold)
