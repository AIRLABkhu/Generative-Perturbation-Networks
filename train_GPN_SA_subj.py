...
def train(param):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(subject_list)):
        ...
        for i in range(num_epoch):
            ...
            for train_x, train_y in train_loader:
                ...
                output = model.forward(train_x_adv.cuda())
                if param.attack_type == 'non-targeted':
            ...
            if i != 0 and (i + 1) % 5 == 0:
                if param.attack_type == 'non-targeted':
                    ...
                    uap_file_name = param.uap_path + 'uap_air_exam_t%d_fold%d_subj.npy' % (param.attack_target, fold)
                    np.save(uap_file_name, uap_exam)
                    print('Saved best perturbation at ' + uap_file_name)
    ...
    if param.attack_type == 'targeted':
        np.savetxt(param.uap_path + '_air_exam_result_target%d_fold_subj.txt' % param.attack_target, result_list, fmt='%1.4f')
        print('saved at' + param.uap_path + '_air_exam_result_target%d_fold_subj.txt'% param.attack_target)
    elif param.attack_type == 'non-targeted':
        np.savetxt(param.uap_path + '_air_exam_result_non_target_fold_subj.txt', result_list, fmt='%1.4f')
        print('saved at' + param.uap_path + '_air_exam_result_non_target_fold_subj.txt')
